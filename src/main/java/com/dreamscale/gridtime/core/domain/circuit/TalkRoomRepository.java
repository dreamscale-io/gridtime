...
public interface TalkRoomRepository extends CrudRepository<TalkRoomEntity, UUID> {
    ...
    List<TalkRoomEntity> findAllRoomsByMembership(@Param("organizationId") UUID organizationId, @Param("memberId") UUID memberId);
    ...
    @Query(nativeQuery = true, value = "select * from talk_room tr  " +
            ...
            "order by tr.room_name ) ")
    List<TalkRoomEntity> findNonTeamRoomsByMembership(@Param("organizationId") UUID organizationId, @Param("memberId") UUID memberId);
