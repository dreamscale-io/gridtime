...
select cm.id, cm.circle_id, c.circle_name, cm.torchie_id, m.full_name, cm.position, cm.message_type, cm.metadata
from circle_message cm
join circle c on cm.circle_id = c.id
join organization_member om on cm.torchie_id = om.id
