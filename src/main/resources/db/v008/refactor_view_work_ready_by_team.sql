...
create view work_ready_by_team_view as
  select wi.team_id, wi.zoom_level, wi.tile_seq, wi.grid_time, wi.work_to_do_type,
         ...
         (SELECT count(tm.member_id)
          FROM team_member tm left outer join torchie_feed_cursor f on (tm.member_id = f.torchie_id)
          WHERE wi.team_id = tm.team_id and (f.torchie_id is null or f.last_published_data_cursor is not null)) as team_size
